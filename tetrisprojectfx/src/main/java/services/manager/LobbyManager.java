...
public class LobbyManager {
	...
	public GameLobby findLobby(){
		try(PreparedStatement prepstmt = con.prepareStatement("select * from game_lobby where id=?")){
			...
			if(result.next()) {
				...
				if(guestId != 0) lobby.setGuest(new PlayerManager().getPlayerById(con, guestId));
				else lobby.setGuest(null);
				if(guestId == userId) isInLobby = true;
	...
	public GameLobby findLobby(int lobbyId){
		try(PreparedStatement prepstmt = con.prepareStatement("select * from game_lobby where id=?")){
			...
			if(result.next()) {
				GameLobby lobby = new GameLobby(result);
				lobby.setHost(new PlayerManager().getPlayerById(con, result.getInt("host_id")));
				if(result.getInt("guest_id") != 0) lobby.setGuest(new PlayerManager().getPlayerById(con, result.getInt("guest_id")));
				else lobby.setGuest(null);
				return lobby;
			}
		} catch (SQLException e) {
	...
	public boolean joinLobby() {
		...
		else sql = "update game_lobby set guest_id=? where id=?";
	...
	public boolean leaveLobby() {
		String sql;
		if(isHost) sql = "delete from game_lobby where host_id = ?";
		else sql = "update game_lobby set guest_id = 0 where guest_id";
		if(!isInLobby) {
			try(PreparedStatement prepstmt = con.prepareStatement(sql)){
				prepstmt.setInt(1, userId);
				if(prepstmt.execute()) {
	...
	public boolean startGame() {
		if(isHost && findLobby().getGuest().getPlayerId() != 1) {
			try(PreparedStatement prepstmt = con.prepareStatement("update game_lobby set has_begun=true where id = ?")){
				prepstmt.setInt(1, lobbyId);
				return prepstmt.execute();
			} catch (SQLException e) {
				e.printStackTrace();
